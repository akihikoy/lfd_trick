...
def ToStr(*lists):
  ...
  delim2= ''
  for v in lists:
    s+= delim2 + (delim.join(map(str,list(v))) if v!=None else '')
    delim2= delim
  return s
...
def WriteVarToFile(x, file_name, except_cnv=lambda y:y):
  x2= ToStdType(x, except_cnv)
  so= sys.stdout
  sys.stdout= OpenW(file_name)
  PrintDict(x2)
  sys.stdout= so
